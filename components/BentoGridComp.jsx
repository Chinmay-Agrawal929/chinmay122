...
export default function BentoGridComp() {
    ...
    const handleApplyClick = (feature) => {
        if (feature.href === "/departments") {
            router.push(feature.href);
        } else {
            setSelectedDept(feature);
        }
    };

    const handleConfirmApply = () => {
        if (selectedDept) {
            router.push(selectedDept.href);
            setSelectedDept(null);
        }
    };

    const popupConfig = selectedDept ? {
        header: `Apply to ${selectedDept.name}`,
        description: `You are about to start your application for the ${selectedDept.name} department.`,
        message: [
            "You can apply to up to two departments in total.",
            "Make sure your profile is complete before submitting."
        ],
        buttons: [
            { label: "Cancel", variant: "outline", onClick: () => setSelectedDept(null) },
            { label: "Proceed to Application", variant: "default", onClick: handleConfirmApply }
        ]
    } : null;
        ...
        <div className="flex flex-col gap-4 p-4 w-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-4 justify-center items-between p-5">
                    {features.slice(0, 5).map((feature) => (
                        <div key={feature.name}>
                            {feature.name !== "null" && (
                                <BentoCard key={feature.name} {...feature} onClick={() => handleApplyClick(feature)} />
                            )}
                        </div>
                    ))}
                </div>
            ...
            <div className="p-5 flex flex-col gap-4">
                ...
                <BentoCard
                    ...
                    key={features[10].name}
                    {...features[10]}
                    onClick={() => handleApplyClick(features[10])}
                />
            </div>
